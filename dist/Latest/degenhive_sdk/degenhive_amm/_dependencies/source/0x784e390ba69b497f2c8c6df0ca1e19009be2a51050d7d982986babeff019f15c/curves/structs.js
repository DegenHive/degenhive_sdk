...
/* ============================== Curved =============================== */
function isCurved(type) { type = (0, util_1.compressSuiType)(type); return type === `${index_1.PKG_V1}::curves::Curved`; }
class Curved {
    ...
    static async fetch(client, id) {
        ...
        return Curved.fromBcs((0, utils_1.fromB64)(res.data.bcs.bcsBytes));
    }
}
exports.Curved = Curved;
Curved.$typeName = `${index_1.PKG_V1}::curves::Curved`;
Curved.$numTypeParams = 0;
Curved.$isPhantom = [];
/* ============================== Stable =============================== */
function isStable(type) { type = (0, util_1.compressSuiType)(type); return type === `${index_1.PKG_V1}::curves::Stable`; }
class Stable {
    constructor(typeArgs, fields) {
        this.__StructClass = true;
    ...
    static reified() { return { typeName: Stable.$typeName, fullTypeName: (0, util_1.composeSuiType)(Stable.$typeName, ...[]), typeArgs: [], isPhantom: Stable.$isPhantom, reifiedTypeArgs: [], fromFields: (fields) => Stable.fromFields(fields), fromFieldsWithTypes: (item) => Stable.fromFieldsWithTypes(item), fromBcs: (data) => Stable.fromBcs(data), bcs: Stable.bcs, fromJSONField: (field) => Stable.fromJSONField(field), fromJSON: (json) => Stable.fromJSON(json), fromSuiParsedData: (content) => Stable.fromSuiParsedData(content), fetch: async (client, id) => Stable.fetch(client, id), new: (fields) => { return new Stable([], fields); }, kind: "StructClassReified", }; }
    ...
    static async fetch(client, id) {
        ...
        return Stable.fromBcs((0, utils_1.fromB64)(res.data.bcs.bcsBytes));
    }
}
exports.Stable = Stable;
Stable.$typeName = `${index_1.PKG_V1}::curves::Stable`;
Stable.$numTypeParams = 0;
Stable.$isPhantom = [];
/* ============================== Weighted =============================== */
function isWeighted(type) { type = (0, util_1.compressSuiType)(type); return type === `${index_1.PKG_V1}::curves::Weighted`; }
class Weighted {
    constructor(typeArgs, fields) {
        this.__StructClass = true;
    ...
    static async fetch(client, id) {
        ...
        return Weighted.fromBcs((0, utils_1.fromB64)(res.data.bcs.bcsBytes));
    }
}
exports.Weighted = Weighted;
Weighted.$typeName = `${index_1.PKG_V1}::curves::Weighted`;
