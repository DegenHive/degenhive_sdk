...
/* ============================== ID =============================== */
function isID(type) { type = (0, util_1.compressSuiType)(type); return type === `${index_1.PKG_V21}::object::ID`; }
class ID {
    ...
    static get bcs() {
        return bcs_1.bcs.struct("ID", {
            bytes: bcs_1.bcs.bytes(32).transform({ input: (val) => (0, utils_1.fromHEX)(val), output: (val) => (0, utils_1.toHEX)(val), })
        });
    }
    ;
    ...
    static async fetch(client, id) {
        ...
        return ID.fromBcs((0, utils_1.fromB64)(res.data.bcs.bcsBytes));
    }
}
exports.ID = ID;
ID.$typeName = `${index_1.PKG_V21}::object::ID`;
ID.$numTypeParams = 0;
ID.$isPhantom = [];
/* ============================== UID =============================== */
function isUID(type) { type = (0, util_1.compressSuiType)(type); return type === `${index_1.PKG_V21}::object::UID`; }
class UID {
    constructor(typeArgs, fields) {
        this.__StructClass = true;
    ...
    static async fetch(client, id) {
        ...
        return UID.fromBcs((0, utils_1.fromB64)(res.data.bcs.bcsBytes));
    }
}
exports.UID = UID;
UID.$typeName = `${index_1.PKG_V21}::object::UID`;
UID.$numTypeParams = 0;
UID.$isPhantom = [];
