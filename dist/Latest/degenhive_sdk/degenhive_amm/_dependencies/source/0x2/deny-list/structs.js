...
/* ============================== DenyList =============================== */
function isDenyList(type) { type = (0, util_1.compressSuiType)(type); return type === `${index_1.PKG_V21}::deny_list::DenyList`; }
class DenyList {
    ...
    static reified() { return { typeName: DenyList.$typeName, fullTypeName: (0, util_1.composeSuiType)(DenyList.$typeName, ...[]), typeArgs: [], isPhantom: DenyList.$isPhantom, reifiedTypeArgs: [], fromFields: (fields) => DenyList.fromFields(fields), fromFieldsWithTypes: (item) => DenyList.fromFieldsWithTypes(item), fromBcs: (data) => DenyList.fromBcs(data), bcs: DenyList.bcs, fromJSONField: (field) => DenyList.fromJSONField(field), fromJSON: (json) => DenyList.fromJSON(json), fromSuiParsedData: (content) => DenyList.fromSuiParsedData(content), fetch: async (client, id) => DenyList.fetch(client, id), new: (fields) => { return new DenyList([], fields); }, kind: "StructClassReified", }; }
    ...
    static async fetch(client, id) {
        ...
        return DenyList.fromBcs((0, utils_1.fromB64)(res.data.bcs.bcsBytes));
    }
}
exports.DenyList = DenyList;
DenyList.$typeName = `${index_1.PKG_V21}::deny_list::DenyList`;
DenyList.$numTypeParams = 0;
DenyList.$isPhantom = [];
/* ============================== PerTypeList =============================== */
function isPerTypeList(type) { type = (0, util_1.compressSuiType)(type); return type === `${index_1.PKG_V21}::deny_list::PerTypeList`; }
class PerTypeList {
    constructor(typeArgs, fields) {
        this.__StructClass = true;
    ...
    static async fetch(client, id) {
        ...
        return PerTypeList.fromBcs((0, utils_1.fromB64)(res.data.bcs.bcsBytes));
    }
}
exports.PerTypeList = PerTypeList;
PerTypeList.$typeName = `${index_1.PKG_V21}::deny_list::PerTypeList`;
