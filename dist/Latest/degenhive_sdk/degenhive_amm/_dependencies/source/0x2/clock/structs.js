...
/* ============================== Clock =============================== */
function isClock(type) { type = (0, util_1.compressSuiType)(type); return type === `${index_1.PKG_V21}::clock::Clock`; }
class Clock {
    ...
    static reified() { return { typeName: Clock.$typeName, fullTypeName: (0, util_1.composeSuiType)(Clock.$typeName, ...[]), typeArgs: [], isPhantom: Clock.$isPhantom, reifiedTypeArgs: [], fromFields: (fields) => Clock.fromFields(fields), fromFieldsWithTypes: (item) => Clock.fromFieldsWithTypes(item), fromBcs: (data) => Clock.fromBcs(data), bcs: Clock.bcs, fromJSONField: (field) => Clock.fromJSONField(field), fromJSON: (json) => Clock.fromJSON(json), fromSuiParsedData: (content) => Clock.fromSuiParsedData(content), fetch: async (client, id) => Clock.fetch(client, id), new: (fields) => { return new Clock([], fields); }, kind: "StructClassReified", }; }
    ...
    static async fetch(client, id) {
        ...
        return Clock.fromBcs((0, utils_1.fromB64)(res.data.bcs.bcsBytes));
    }
}
exports.Clock = Clock;
Clock.$typeName = `${index_1.PKG_V21}::clock::Clock`;
