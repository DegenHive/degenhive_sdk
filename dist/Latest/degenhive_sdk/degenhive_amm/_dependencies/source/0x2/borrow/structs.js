...
/* ============================== Borrow =============================== */
function isBorrow(type) { type = (0, util_1.compressSuiType)(type); return type === `${index_1.PKG_V21}::borrow::Borrow`; }
class Borrow {
    ...
    static get bcs() {
        return bcs_1.bcs.struct("Borrow", {
            ref: bcs_1.bcs.bytes(32).transform({ input: (val) => (0, utils_1.fromHEX)(val), output: (val) => (0, utils_1.toHEX)(val), }), obj: structs_2.ID.bcs
        });
    }
    ;
...
Borrow.$typeName = `${index_1.PKG_V21}::borrow::Borrow`;
Borrow.$numTypeParams = 0;
Borrow.$isPhantom = [];
/* ============================== Referent =============================== */
function isReferent(type) { type = (0, util_1.compressSuiType)(type); return type.startsWith(`${index_1.PKG_V21}::borrow::Referent` + '<'); }
class Referent {
    constructor(typeArgs, fields) {
        this.__StructClass = true;
    ...
    static get bcs() {
        return (T) => bcs_1.bcs.struct(`Referent<${T.name}>`, {
            id: bcs_1.bcs.bytes(32).transform({ input: (val) => (0, utils_1.fromHEX)(val), output: (val) => (0, utils_1.toHEX)(val), }), value: structs_1.Option.bcs(T)
        });
    }
    ;
    ...
    static async fetch(client, typeArg, id) {
        ...
        ;
        return Referent.fromBcs(typeArg, (0, utils_1.fromB64)(res.data.bcs.bcsBytes));
    }
}
exports.Referent = Referent;
Referent.$typeName = `${index_1.PKG_V21}::borrow::Referent`;
