...
/* ============================== VersionChangeCap =============================== */
function isVersionChangeCap(type) { type = (0, util_1.compressSuiType)(type); return type === `${index_1.PKG_V21}::versioned::VersionChangeCap`; }
class VersionChangeCap {
    ...
    static reified() { return { typeName: VersionChangeCap.$typeName, fullTypeName: (0, util_1.composeSuiType)(VersionChangeCap.$typeName, ...[]), typeArgs: [], isPhantom: VersionChangeCap.$isPhantom, reifiedTypeArgs: [], fromFields: (fields) => VersionChangeCap.fromFields(fields), fromFieldsWithTypes: (item) => VersionChangeCap.fromFieldsWithTypes(item), fromBcs: (data) => VersionChangeCap.fromBcs(data), bcs: VersionChangeCap.bcs, fromJSONField: (field) => VersionChangeCap.fromJSONField(field), fromJSON: (json) => VersionChangeCap.fromJSON(json), fromSuiParsedData: (content) => VersionChangeCap.fromSuiParsedData(content), fetch: async (client, id) => VersionChangeCap.fetch(client, id), new: (fields) => { return new VersionChangeCap([], fields); }, kind: "StructClassReified", }; }
    ...
    static async fetch(client, id) {
        ...
        return VersionChangeCap.fromBcs((0, utils_1.fromB64)(res.data.bcs.bcsBytes));
    }
}
exports.VersionChangeCap = VersionChangeCap;
VersionChangeCap.$typeName = `${index_1.PKG_V21}::versioned::VersionChangeCap`;
VersionChangeCap.$numTypeParams = 0;
VersionChangeCap.$isPhantom = [];
/* ============================== Versioned =============================== */
function isVersioned(type) { type = (0, util_1.compressSuiType)(type); return type === `${index_1.PKG_V21}::versioned::Versioned`; }
class Versioned {
    constructor(typeArgs, fields) {
        this.__StructClass = true;
    ...
    static async fetch(client, id) {
        ...
        return Versioned.fromBcs((0, utils_1.fromB64)(res.data.bcs.bcsBytes));
    }
}
exports.Versioned = Versioned;
Versioned.$typeName = `${index_1.PKG_V21}::versioned::Versioned`;
