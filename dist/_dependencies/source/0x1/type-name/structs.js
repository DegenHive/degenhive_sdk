...
/* ============================== TypeName =============================== */
function isTypeName(type) { type = (0, util_1.compressSuiType)(type); return type === `${index_1.PKG_V11}::type_name::TypeName`; }
class TypeName {
    ...
    static reified() { return { typeName: TypeName.$typeName, fullTypeName: (0, util_1.composeSuiType)(TypeName.$typeName, ...[]), typeArgs: [], isPhantom: TypeName.$isPhantom, reifiedTypeArgs: [], fromFields: (fields) => TypeName.fromFields(fields), fromFieldsWithTypes: (item) => TypeName.fromFieldsWithTypes(item), fromBcs: (data) => TypeName.fromBcs(data), bcs: TypeName.bcs, fromJSONField: (field) => TypeName.fromJSONField(field), fromJSON: (json) => TypeName.fromJSON(json), fromSuiParsedData: (content) => TypeName.fromSuiParsedData(content), fromSuiObjectData: (content) => TypeName.fromSuiObjectData(content), fetch: async (client, id) => TypeName.fetch(client, id), new: (fields) => { return new TypeName([], fields); }, kind: "StructClassReified", }; }
    ...
    static fromSuiObjectData(data) {
        if (data.bcs) {
            ...
            return TypeName.fromBcs((0, bcs_1.fromB64)(data.bcs.bcsBytes));
        }
        if (data.content) {
