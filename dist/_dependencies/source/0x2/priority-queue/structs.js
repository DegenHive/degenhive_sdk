...
/* ============================== Entry =============================== */
function isEntry(type) { type = (0, util_1.compressSuiType)(type); return type.startsWith(`${index_1.PKG_V28}::priority_queue::Entry` + '<'); }
class Entry {
    ...
    static fromSuiObjectData(typeArg, data) {
        if (data.bcs) {
            ...
            ;
            return Entry.fromBcs(typeArg, (0, bcs_1.fromB64)(data.bcs.bcsBytes));
        }
        if (data.content) {
...
exports.Entry = Entry;
Entry.$typeName = `${index_1.PKG_V28}::priority_queue::Entry`;
Entry.$numTypeParams = 1;
Entry.$isPhantom = [false,];
/* ============================== PriorityQueue =============================== */
function isPriorityQueue(type) { type = (0, util_1.compressSuiType)(type); return type.startsWith(`${index_1.PKG_V28}::priority_queue::PriorityQueue` + '<'); }
class PriorityQueue {
    constructor(typeArgs, fields) {
        this.__StructClass = true;
    ...
    static fromSuiObjectData(typeArg, data) {
        if (data.bcs) {
            ...
            ;
            return PriorityQueue.fromBcs(typeArg, (0, bcs_1.fromB64)(data.bcs.bcsBytes));
        }
        if (data.content) {
...
PriorityQueue.$typeName = `${index_1.PKG_V28}::priority_queue::PriorityQueue`;
