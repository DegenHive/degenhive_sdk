...
/* ============================== WeightedConfig =============================== */
function isWeightedConfig(type) { type = (0, util_1.compressSuiType)(type); return type === `${PKG_V1}::weighted_math::WeightedConfig`; }
class WeightedConfig {
    constructor(typeArgs, fields) {
        this.__StructClass = true;
        this.$typeName = WeightedConfig.$typeName;
        this.$isPhantom = WeightedConfig.$isPhantom;
    ...
    static reified() { return { typeName: WeightedConfig.$typeName, fullTypeName: (0, util_1.composeSuiType)(WeightedConfig.$typeName, ...[]), typeArgs: [], isPhantom: WeightedConfig.$isPhantom, reifiedTypeArgs: [], fromFields: (fields) => WeightedConfig.fromFields(fields), fromFieldsWithTypes: (item) => WeightedConfig.fromFieldsWithTypes(item), fromBcs: (data) => WeightedConfig.fromBcs(data), bcs: WeightedConfig.bcs, fromJSONField: (field) => WeightedConfig.fromJSONField(field), fromJSON: (json) => WeightedConfig.fromJSON(json), fromSuiParsedData: (content) => WeightedConfig.fromSuiParsedData(content), fromSuiObjectData: (content) => WeightedConfig.fromSuiObjectData(content), fetch: async (client, id) => WeightedConfig.fetch(client, id), new: (fields) => { return new WeightedConfig([], fields); }, kind: "StructClassReified", }; }
    ...
    static fromSuiObjectData(data) {
        if (data.bcs) {
            if (data.bcs.dataType !== "moveObject" || !isWeightedConfig(data.bcs.type)) {
                throw new Error(`object at is not a WeightedConfig object`);
            }
            return WeightedConfig.fromBcs((0, bcs_1.fromB64)(data.bcs.bcsBytes));
        }
        if (data.content) {
            return WeightedConfig.fromSuiParsedData(data.content);
        }
...
WeightedConfig.$typeName = `${PKG_V1}::weighted_math::WeightedConfig`;
WeightedConfig.$numTypeParams = 0;
WeightedConfig.$isPhantom = [];
