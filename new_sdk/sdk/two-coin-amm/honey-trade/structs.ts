...
export class HoneyBurnt implements StructClass { __StructClass = true as const;
 ...
 private constructor(typeArgs: [], fields: HoneyBurntFields, ) { this.$fullTypeName = composeSuiType( HoneyBurnt.$typeName, ...typeArgs ) as `${typeof PKG_V1}::honey_trade::HoneyBurnt`; this.$typeArgs = typeArgs;
 ...
 static get bcs() { return bcs.struct("HoneyBurnt", {
 ...
 static fromFieldsWithTypes( item: FieldsWithTypes ): HoneyBurnt { if (!isHoneyBurnt(item.type)) { throw new Error("not a HoneyBurnt type");
 ...
 static fromJSON( json: Record<string, any> ): HoneyBurnt { if (json.$typeName !== HoneyBurnt.$typeName) { throw new Error("not a WithTwoGenerics json object") };
 ...
 static fromSuiObjectData( data: SuiObjectData ): HoneyBurnt { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isHoneyBurnt(data.bcs.type)) { throw new Error(`object at is not a HoneyBurnt object`); }

 return HoneyBurnt.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return HoneyBurnt.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }

 static async fetch( client: SuiClient, id: string ): Promise<HoneyBurnt> { const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching HoneyBurnt object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isHoneyBurnt(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a HoneyBurnt object`); }
...
export class HoneyManager<T extends PhantomTypeArgument> implements StructClass { __StructClass = true as const;
 ...
 private constructor(typeArgs: [PhantomToTypeStr<T>], fields: HoneyManagerFields<T>, ) { this.$fullTypeName = composeSuiType( HoneyManager.$typeName, ...typeArgs ) as `${typeof PKG_V1}::honey_trade::HoneyManager<${PhantomToTypeStr<T>}>`; this.$typeArgs = typeArgs;
 ...
 static get bcs() { return bcs.struct("HoneyManager", {
 ...
 static fromFieldsWithTypes<T extends PhantomReified<PhantomTypeArgument>>( typeArg: T, item: FieldsWithTypes ): HoneyManager<ToPhantomTypeArgument<T>> { if (!isHoneyManager(item.type)) { throw new Error("not a HoneyManager type");
 ...
 static fromJSON<T extends PhantomReified<PhantomTypeArgument>>( typeArg: T, json: Record<string, any> ): HoneyManager<ToPhantomTypeArgument<T>> { if (json.$typeName !== HoneyManager.$typeName) { throw new Error("not a WithTwoGenerics json object") }; assertReifiedTypeArgsMatch( composeSuiType(HoneyManager.$typeName, extractType(typeArg)), json.$typeArgs, [typeArg], )
 ...
 static fromSuiObjectData<T extends PhantomReified<PhantomTypeArgument>>( typeArg: T, data: SuiObjectData ): HoneyManager<ToPhantomTypeArgument<T>> { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isHoneyManager(data.bcs.type)) { throw new Error(`object at is not a HoneyManager object`); }

 const gotTypeArgs = parseTypeName(data.bcs.type).typeArgs; if (gotTypeArgs.length !== 1) { throw new Error(`type argument mismatch: expected 1 type argument but got '${gotTypeArgs.length}'`); }; const gotTypeArg = compressSuiType(gotTypeArgs[0]); const expectedTypeArg = compressSuiType(extractType(typeArg)); if (gotTypeArg !== compressSuiType(extractType(typeArg))) { throw new Error(`type argument mismatch: expected '${expectedTypeArg}' but got '${gotTypeArg}'`); };

 return HoneyManager.fromBcs( typeArg, fromB64(data.bcs.bcsBytes) ); } if (data.content) { return HoneyManager.fromSuiParsedData( typeArg, data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }

 static async fetch<T extends PhantomReified<PhantomTypeArgument>>( client: SuiClient, typeArg: T, id: string ): Promise<HoneyManager<ToPhantomTypeArgument<T>>> { const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching HoneyManager object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isHoneyManager(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a HoneyManager object`); }
