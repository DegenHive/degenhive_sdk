...
/* ============================== BurnTokensCapability =============================== */

export function isBurnTokensCapability(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V1}::two_pool::BurnTokensCapability`; }

export interface BurnTokensCapabilityFields { id: ToField<UID> }

export type BurnTokensCapabilityReified = Reified< BurnTokensCapability, BurnTokensCapabilityFields >;

export class BurnTokensCapability implements StructClass { __StructClass = true as const;

 static readonly $typeName = `${PKG_V1}::two_pool::BurnTokensCapability`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

 readonly $typeName = BurnTokensCapability.$typeName; readonly $fullTypeName: `${typeof PKG_V1}::two_pool::BurnTokensCapability`; readonly $typeArgs: []; readonly $isPhantom = BurnTokensCapability.$isPhantom;

 readonly id: ToField<UID>

 private constructor(typeArgs: [], fields: BurnTokensCapabilityFields, ) { this.$fullTypeName = composeSuiType( BurnTokensCapability.$typeName, ...typeArgs ) as `${typeof PKG_V1}::two_pool::BurnTokensCapability`; this.$typeArgs = typeArgs;

 this.id = fields.id; }

 static reified( ): BurnTokensCapabilityReified { return { typeName: BurnTokensCapability.$typeName, fullTypeName: composeSuiType( BurnTokensCapability.$typeName, ...[] ) as `${typeof PKG_V1}::two_pool::BurnTokensCapability`, typeArgs: [ ] as [], isPhantom: BurnTokensCapability.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => BurnTokensCapability.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => BurnTokensCapability.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => BurnTokensCapability.fromBcs( data, ), bcs: BurnTokensCapability.bcs, fromJSONField: (field: any) => BurnTokensCapability.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => BurnTokensCapability.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => BurnTokensCapability.fromSuiParsedData( content, ), fromSuiObjectData: (content: SuiObjectData) => BurnTokensCapability.fromSuiObjectData( content, ), fetch: async (client: SuiClient, id: string) => BurnTokensCapability.fetch( client, id, ), new: ( fields: BurnTokensCapabilityFields, ) => { return new BurnTokensCapability( [], fields ) }, kind: "StructClassReified", } }

 static get r() { return BurnTokensCapability.reified() }

 static phantom( ): PhantomReified<ToTypeStr<BurnTokensCapability>> { return phantom(BurnTokensCapability.reified( )); } static get p() { return BurnTokensCapability.phantom() }

 static get bcs() { return bcs.struct("BurnTokensCapability", {

 id: UID.bcs

}) };

 static fromFields( fields: Record<string, any> ): BurnTokensCapability { return BurnTokensCapability.reified( ).new( { id: decodeFromFields(UID.reified(), fields.id) } ) }

 static fromFieldsWithTypes( item: FieldsWithTypes ): BurnTokensCapability { if (!isBurnTokensCapability(item.type)) { throw new Error("not a BurnTokensCapability type");

 }

 return BurnTokensCapability.reified( ).new( { id: decodeFromFieldsWithTypes(UID.reified(), item.fields.id) } ) }

 static fromBcs( data: Uint8Array ): BurnTokensCapability { return BurnTokensCapability.fromFields( BurnTokensCapability.bcs.parse(data) ) }
 ...
 static fromJSONField( field: any ): BurnTokensCapability { return BurnTokensCapability.reified( ).new( { id: decodeFromJSONField(UID.reified(), field.id) } ) }

 static fromJSON( json: Record<string, any> ): BurnTokensCapability { if (json.$typeName !== BurnTokensCapability.$typeName) { throw new Error("not a WithTwoGenerics json object") };

 return BurnTokensCapability.fromJSONField( json, ) }

 static fromSuiParsedData( content: SuiParsedData ): BurnTokensCapability { if (content.dataType !== "moveObject") { throw new Error("not an object"); } if (!isBurnTokensCapability(content.type)) { throw new Error(`object at ${(content.fields as any).id} is not a BurnTokensCapability object`); } return BurnTokensCapability.fromFieldsWithTypes( content ); }

 static fromSuiObjectData( data: SuiObjectData ): BurnTokensCapability { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isBurnTokensCapability(data.bcs.type)) { throw new Error(`object at is not a BurnTokensCapability object`); }

 return BurnTokensCapability.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return BurnTokensCapability.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }

 static async fetch( client: SuiClient, id: string ): Promise<BurnTokensCapability> { const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching BurnTokensCapability object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isBurnTokensCapability(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a BurnTokensCapability object`); }

 return BurnTokensCapability.fromSuiObjectData( res.data ); }
