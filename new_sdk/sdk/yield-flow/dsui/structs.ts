...
export class DSUI implements StructClass { __StructClass = true as const;
 ...
 private constructor(typeArgs: [], fields: DSUIFields, ) { this.$fullTypeName = composeSuiType( DSUI.$typeName, ...typeArgs ) as `${typeof PKG_V1}::dsui::DSUI`; this.$typeArgs = typeArgs;
 ...
 static fromSuiObjectData( data: SuiObjectData ): DSUI { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isDSUI(data.bcs.type)) { throw new Error(`object at is not a DSUI object`); }

 return DSUI.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return DSUI.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
