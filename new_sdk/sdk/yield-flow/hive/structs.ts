...
export class HIVE implements StructClass {
    ...
    private constructor(typeArgs: [], fields: HIVEFields,) {
        this.$fullTypeName = composeSuiType(HIVE.$typeName, ...typeArgs) as `${typeof PKG_V1}::hive::HIVE`; this.$typeArgs = typeArgs;
    ...
    static fromFieldsWithTypes(item: FieldsWithTypes): HIVE {
        if (!isHIVE(item.type)) {
    ...
    static fromJSON(json: Record<string, any>): HIVE {
        if (json.$typeName !== HIVE.$typeName) { throw new Error("not a WithTwoGenerics json object") };
    ...
    static fromSuiObjectData(data: SuiObjectData): HIVE {
        if (data.bcs) {
            if (data.bcs.dataType !== "moveObject" || !isHIVE(data.bcs.type)) { throw new Error(`object at is not a HIVE object`); }

            return HIVE.fromBcs(fromB64(data.bcs.bcsBytes));
        } if (data.content) { return HIVE.fromSuiParsedData(data.content) } throw new Error("Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.");
    }

    static async fetch(client: SuiClient, id: string): Promise<HIVE> {
        const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching HIVE object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isHIVE(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a HIVE object`); }
