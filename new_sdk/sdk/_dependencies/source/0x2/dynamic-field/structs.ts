...
export function isField(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V28}::dynamic_field::Field` + '<'); }
...
export class Field<Name extends TypeArgument, Value extends TypeArgument> implements StructClass {
    ...
    static readonly $typeName = `${PKG_V28}::dynamic_field::Field`; static readonly $numTypeParams = 2; static readonly $isPhantom = [false, false,] as const;

    readonly $typeName = Field.$typeName; readonly $fullTypeName: `${typeof PKG_V28}::dynamic_field::Field<${ToTypeStr<Name>}, ${ToTypeStr<Value>}>`; readonly $typeArgs: [ToTypeStr<Name>, ToTypeStr<Value>]; readonly $isPhantom = Field.$isPhantom;
    ...
    static fromJSON<Name extends Reified<TypeArgument, any>, Value extends Reified<TypeArgument, any>>(typeArgs: [Name, Value], json: Record<string, any>): Field<ToTypeArgument<Name>, ToTypeArgument<Value>> {
        if (json.$typeName !== Field.$typeName) { throw new Error("not a WithTwoGenerics json object") }; assertReifiedTypeArgsMatch(composeSuiType(Field.$typeName, ...typeArgs.map(extractType)), json.$typeArgs, typeArgs,)
    ...
    static fromSuiObjectData<Name extends Reified<TypeArgument, any>, Value extends Reified<TypeArgument, any>>(typeArgs: [Name, Value], data: SuiObjectData): Field<ToTypeArgument<Name>, ToTypeArgument<Value>> {
        if (data.bcs) {
            if (data.bcs.dataType !== "moveObject" || !isField(data.bcs.type)) { throw new Error(`object at is not a Field object`); }
        ...
        } if (data.content) { return Field.fromSuiParsedData(typeArgs, data.content) } throw new Error("Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.");
    }

    static async fetch<Name extends Reified<TypeArgument, any>, Value extends Reified<TypeArgument, any>>(client: SuiClient, typeArgs: [Name, Value], id: string): Promise<Field<ToTypeArgument<Name>, ToTypeArgument<Value>>> {
        const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching Field object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isField(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a Field object`); }
