...
export function isActiveJwk(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V28}::authenticator_state::ActiveJwk`; }
...
export class ActiveJwk implements StructClass {
    ...
    static readonly $typeName = `${PKG_V28}::authenticator_state::ActiveJwk`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

    readonly $typeName = ActiveJwk.$typeName; readonly $fullTypeName: `${typeof PKG_V28}::authenticator_state::ActiveJwk`; readonly $typeArgs: []; readonly $isPhantom = ActiveJwk.$isPhantom;
    ...
    static fromJSON(json: Record<string, any>): ActiveJwk {
        if (json.$typeName !== ActiveJwk.$typeName) { throw new Error("not a WithTwoGenerics json object") };
    ...
    static fromSuiObjectData(data: SuiObjectData): ActiveJwk {
        if (data.bcs) {
            if (data.bcs.dataType !== "moveObject" || !isActiveJwk(data.bcs.type)) { throw new Error(`object at is not a ActiveJwk object`); }

            return ActiveJwk.fromBcs(fromB64(data.bcs.bcsBytes));
        } if (data.content) { return ActiveJwk.fromSuiParsedData(data.content) } throw new Error("Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.");
    }

    static async fetch(client: SuiClient, id: string): Promise<ActiveJwk> {
        const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching ActiveJwk object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isActiveJwk(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a ActiveJwk object`); }
...
export function isAuthenticatorState(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V28}::authenticator_state::AuthenticatorState`; }
...
export class AuthenticatorState implements StructClass {
    ...
    static fromJSON(json: Record<string, any>): AuthenticatorState {
        if (json.$typeName !== AuthenticatorState.$typeName) { throw new Error("not a WithTwoGenerics json object") };
    ...
    static fromSuiObjectData(data: SuiObjectData): AuthenticatorState {
        if (data.bcs) {
            if (data.bcs.dataType !== "moveObject" || !isAuthenticatorState(data.bcs.type)) { throw new Error(`object at is not a AuthenticatorState object`); }

            return AuthenticatorState.fromBcs(fromB64(data.bcs.bcsBytes));
        } if (data.content) { return AuthenticatorState.fromSuiParsedData(data.content) } throw new Error("Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.");
    }

    static async fetch(client: SuiClient, id: string): Promise<AuthenticatorState> {
        const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching AuthenticatorState object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isAuthenticatorState(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a AuthenticatorState object`); }
...
export function isAuthenticatorStateInner(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V28}::authenticator_state::AuthenticatorStateInner`; }
...
export class AuthenticatorStateInner implements StructClass {
    ...
    static readonly $typeName = `${PKG_V28}::authenticator_state::AuthenticatorStateInner`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

    readonly $typeName = AuthenticatorStateInner.$typeName; readonly $fullTypeName: `${typeof PKG_V28}::authenticator_state::AuthenticatorStateInner`; readonly $typeArgs: []; readonly $isPhantom = AuthenticatorStateInner.$isPhantom;
    ...
    static fromJSON(json: Record<string, any>): AuthenticatorStateInner {
        if (json.$typeName !== AuthenticatorStateInner.$typeName) { throw new Error("not a WithTwoGenerics json object") };
    ...
    static fromSuiObjectData(data: SuiObjectData): AuthenticatorStateInner {
        if (data.bcs) {
            if (data.bcs.dataType !== "moveObject" || !isAuthenticatorStateInner(data.bcs.type)) { throw new Error(`object at is not a AuthenticatorStateInner object`); }

            return AuthenticatorStateInner.fromBcs(fromB64(data.bcs.bcsBytes));
        } if (data.content) { return AuthenticatorStateInner.fromSuiParsedData(data.content) } throw new Error("Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.");
    }
...
export function isJWK(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V28}::authenticator_state::JWK`; }
...
export class JWK implements StructClass {
    ...
    static readonly $typeName = `${PKG_V28}::authenticator_state::JWK`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

    readonly $typeName = JWK.$typeName; readonly $fullTypeName: `${typeof PKG_V28}::authenticator_state::JWK`; readonly $typeArgs: []; readonly $isPhantom = JWK.$isPhantom;
    ...
    private constructor(typeArgs: [], fields: JWKFields,) {
        this.$fullTypeName = composeSuiType(JWK.$typeName, ...typeArgs) as `${typeof PKG_V28}::authenticator_state::JWK`; this.$typeArgs = typeArgs;

        this.kty = fields.kty;; this.e = fields.e;; this.n = fields.n;; this.alg = fields.alg;
    }

    static reified(): JWKReified { return { typeName: JWK.$typeName, fullTypeName: composeSuiType(JWK.$typeName, ...[]) as `${typeof PKG_V28}::authenticator_state::JWK`, typeArgs: [] as [], isPhantom: JWK.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => JWK.fromFields(fields,), fromFieldsWithTypes: (item: FieldsWithTypes) => JWK.fromFieldsWithTypes(item,), fromBcs: (data: Uint8Array) => JWK.fromBcs(data,), bcs: JWK.bcs, fromJSONField: (field: any) => JWK.fromJSONField(field,), fromJSON: (json: Record<string, any>) => JWK.fromJSON(json,), fromSuiParsedData: (content: SuiParsedData) => JWK.fromSuiParsedData(content,), fromSuiObjectData: (content: SuiObjectData) => JWK.fromSuiObjectData(content,), fetch: async (client: SuiClient, id: string) => JWK.fetch(client, id,), new: (fields: JWKFields,) => { return new JWK([], fields) }, kind: "StructClassReified", } }
    ...
    static fromJSON(json: Record<string, any>): JWK {
        if (json.$typeName !== JWK.$typeName) { throw new Error("not a WithTwoGenerics json object") };
    ...
    static fromSuiObjectData(data: SuiObjectData): JWK {
        if (data.bcs) {
            if (data.bcs.dataType !== "moveObject" || !isJWK(data.bcs.type)) { throw new Error(`object at is not a JWK object`); }

            return JWK.fromBcs(fromB64(data.bcs.bcsBytes));
        } if (data.content) { return JWK.fromSuiParsedData(data.content) } throw new Error("Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.");
    }

    static async fetch(client: SuiClient, id: string): Promise<JWK> {
        const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching JWK object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isJWK(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a JWK object`); }
...
export function isJwkId(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V28}::authenticator_state::JwkId`; }
...
export class JwkId implements StructClass {
    ...
    static readonly $typeName = `${PKG_V28}::authenticator_state::JwkId`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

    readonly $typeName = JwkId.$typeName; readonly $fullTypeName: `${typeof PKG_V28}::authenticator_state::JwkId`; readonly $typeArgs: []; readonly $isPhantom = JwkId.$isPhantom;

    readonly iss: ToField<String>; readonly kid: ToField<String>

    private constructor(typeArgs: [], fields: JwkIdFields,) {
        this.$fullTypeName = composeSuiType(JwkId.$typeName, ...typeArgs) as `${typeof PKG_V28}::authenticator_state::JwkId`; this.$typeArgs = typeArgs;

        this.iss = fields.iss;; this.kid = fields.kid;
    }

    static reified(): JwkIdReified { return { typeName: JwkId.$typeName, fullTypeName: composeSuiType(JwkId.$typeName, ...[]) as `${typeof PKG_V28}::authenticator_state::JwkId`, typeArgs: [] as [], isPhantom: JwkId.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => JwkId.fromFields(fields,), fromFieldsWithTypes: (item: FieldsWithTypes) => JwkId.fromFieldsWithTypes(item,), fromBcs: (data: Uint8Array) => JwkId.fromBcs(data,), bcs: JwkId.bcs, fromJSONField: (field: any) => JwkId.fromJSONField(field,), fromJSON: (json: Record<string, any>) => JwkId.fromJSON(json,), fromSuiParsedData: (content: SuiParsedData) => JwkId.fromSuiParsedData(content,), fromSuiObjectData: (content: SuiObjectData) => JwkId.fromSuiObjectData(content,), fetch: async (client: SuiClient, id: string) => JwkId.fetch(client, id,), new: (fields: JwkIdFields,) => { return new JwkId([], fields) }, kind: "StructClassReified", } }
    ...
    static fromJSON(json: Record<string, any>): JwkId {
        if (json.$typeName !== JwkId.$typeName) { throw new Error("not a WithTwoGenerics json object") };
    ...
    static fromSuiObjectData(data: SuiObjectData): JwkId {
        if (data.bcs) {
            if (data.bcs.dataType !== "moveObject" || !isJwkId(data.bcs.type)) { throw new Error(`object at is not a JwkId object`); }

            return JwkId.fromBcs(fromB64(data.bcs.bcsBytes));
        } if (data.content) { return JwkId.fromSuiParsedData(data.content) } throw new Error("Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.");
    }

    static async fetch(client: SuiClient, id: string): Promise<JwkId> {
        const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching JwkId object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isJwkId(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a JwkId object`); }
