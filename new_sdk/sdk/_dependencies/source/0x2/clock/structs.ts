...
export function isClock(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V28}::clock::Clock`; }
...
export class Clock implements StructClass {
    ...
    static readonly $typeName = `${PKG_V28}::clock::Clock`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

    readonly $typeName = Clock.$typeName; readonly $fullTypeName: `${typeof PKG_V28}::clock::Clock`; readonly $typeArgs: []; readonly $isPhantom = Clock.$isPhantom;

    readonly id: ToField<UID>; readonly timestampMs: ToField<"u64">

    private constructor(typeArgs: [], fields: ClockFields,) {
        this.$fullTypeName = composeSuiType(Clock.$typeName, ...typeArgs) as `${typeof PKG_V28}::clock::Clock`; this.$typeArgs = typeArgs;

        this.id = fields.id;; this.timestampMs = fields.timestampMs;
    }

    static reified(): ClockReified { return { typeName: Clock.$typeName, fullTypeName: composeSuiType(Clock.$typeName, ...[]) as `${typeof PKG_V28}::clock::Clock`, typeArgs: [] as [], isPhantom: Clock.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Clock.fromFields(fields,), fromFieldsWithTypes: (item: FieldsWithTypes) => Clock.fromFieldsWithTypes(item,), fromBcs: (data: Uint8Array) => Clock.fromBcs(data,), bcs: Clock.bcs, fromJSONField: (field: any) => Clock.fromJSONField(field,), fromJSON: (json: Record<string, any>) => Clock.fromJSON(json,), fromSuiParsedData: (content: SuiParsedData) => Clock.fromSuiParsedData(content,), fromSuiObjectData: (content: SuiObjectData) => Clock.fromSuiObjectData(content,), fetch: async (client: SuiClient, id: string) => Clock.fetch(client, id,), new: (fields: ClockFields,) => { return new Clock([], fields) }, kind: "StructClassReified", } }
    ...
    static fromJSON(json: Record<string, any>): Clock {
        if (json.$typeName !== Clock.$typeName) { throw new Error("not a WithTwoGenerics json object") };
    ...
    static fromSuiObjectData(data: SuiObjectData): Clock {
        if (data.bcs) {
            if (data.bcs.dataType !== "moveObject" || !isClock(data.bcs.type)) { throw new Error(`object at is not a Clock object`); }

            return Clock.fromBcs(fromB64(data.bcs.bcsBytes));
        } if (data.content) { return Clock.fromSuiParsedData(data.content) } throw new Error("Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.");
    }

    static async fetch(client: SuiClient, id: string): Promise<Clock> {
        const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching Clock object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isClock(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a Clock object`); }
