...
export class CurvedPoolConfig implements StructClass { __StructClass = true as const;
 ...
 static fromSuiObjectData( data: SuiObjectData ): CurvedPoolConfig { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isCurvedPoolConfig(data.bcs.type)) { throw new Error(`object at is not a CurvedPoolConfig object`); }

 return CurvedPoolConfig.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return CurvedPoolConfig.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
