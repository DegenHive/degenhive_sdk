...
export class StablePoolConfig implements StructClass { __StructClass = true as const;
 ...
 private constructor(typeArgs: [], fields: StablePoolConfigFields, ) { this.$fullTypeName = composeSuiType( StablePoolConfig.$typeName, ...typeArgs ) as `${typeof PKG_V1}::stable_math::StablePoolConfig`; this.$typeArgs = typeArgs;
 ...
 static fromSuiObjectData( data: SuiObjectData ): StablePoolConfig { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isStablePoolConfig(data.bcs.type)) { throw new Error(`object at is not a StablePoolConfig object`); }

 return StablePoolConfig.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return StablePoolConfig.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
