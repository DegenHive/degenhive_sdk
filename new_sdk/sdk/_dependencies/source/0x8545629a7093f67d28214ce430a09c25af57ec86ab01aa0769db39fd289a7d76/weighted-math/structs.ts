...
export class WeightedConfig implements StructClass { __StructClass = true as const;
 ...
 static fromSuiObjectData( data: SuiObjectData ): WeightedConfig { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isWeightedConfig(data.bcs.type)) { throw new Error(`object at is not a WeightedConfig object`); }

 return WeightedConfig.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return WeightedConfig.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
