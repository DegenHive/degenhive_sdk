...
export class SuiSystemState implements StructClass {
    ...
    private constructor(typeArgs: [], fields: SuiSystemStateFields,) {
        this.$fullTypeName = composeSuiType(SuiSystemState.$typeName, ...typeArgs) as `${typeof PKG_V17}::sui_system::SuiSystemState`; this.$typeArgs = typeArgs;
    ...
    static fromJSON(json: Record<string, any>): SuiSystemState {
        if (json.$typeName !== SuiSystemState.$typeName) { throw new Error("not a WithTwoGenerics json object") };
    ...
    static fromSuiObjectData(data: SuiObjectData): SuiSystemState {
        if (data.bcs) {
            if (data.bcs.dataType !== "moveObject" || !isSuiSystemState(data.bcs.type)) { throw new Error(`object at is not a SuiSystemState object`); }

            return SuiSystemState.fromBcs(fromB64(data.bcs.bcsBytes));
        } if (data.content) { return SuiSystemState.fromSuiParsedData(data.content) } throw new Error("Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.");
    }

    static async fetch(client: SuiClient, id: string): Promise<SuiSystemState> {
        const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching SuiSystemState object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isSuiSystemState(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a SuiSystemState object`); }
