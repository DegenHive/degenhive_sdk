...
export function isBitVector(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V11}::bit_vector::BitVector`; }
...
export class BitVector implements StructClass {
    ...
    static readonly $typeName = `${PKG_V11}::bit_vector::BitVector`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

    readonly $typeName = BitVector.$typeName; readonly $fullTypeName: `${typeof PKG_V11}::bit_vector::BitVector`; readonly $typeArgs: []; readonly $isPhantom = BitVector.$isPhantom;

    readonly length: ToField<"u64">; readonly bitField: ToField<Vector<"bool">>

    private constructor(typeArgs: [], fields: BitVectorFields,) {
        this.$fullTypeName = composeSuiType(BitVector.$typeName, ...typeArgs) as `${typeof PKG_V11}::bit_vector::BitVector`; this.$typeArgs = typeArgs;

        this.length = fields.length;; this.bitField = fields.bitField;
    }

    static reified(): BitVectorReified { return { typeName: BitVector.$typeName, fullTypeName: composeSuiType(BitVector.$typeName, ...[]) as `${typeof PKG_V11}::bit_vector::BitVector`, typeArgs: [] as [], isPhantom: BitVector.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => BitVector.fromFields(fields,), fromFieldsWithTypes: (item: FieldsWithTypes) => BitVector.fromFieldsWithTypes(item,), fromBcs: (data: Uint8Array) => BitVector.fromBcs(data,), bcs: BitVector.bcs, fromJSONField: (field: any) => BitVector.fromJSONField(field,), fromJSON: (json: Record<string, any>) => BitVector.fromJSON(json,), fromSuiParsedData: (content: SuiParsedData) => BitVector.fromSuiParsedData(content,), fromSuiObjectData: (content: SuiObjectData) => BitVector.fromSuiObjectData(content,), fetch: async (client: SuiClient, id: string) => BitVector.fetch(client, id,), new: (fields: BitVectorFields,) => { return new BitVector([], fields) }, kind: "StructClassReified", } }
    ...
    static fromJSON(json: Record<string, any>): BitVector {
        if (json.$typeName !== BitVector.$typeName) { throw new Error("not a WithTwoGenerics json object") };
    ...
    static fromSuiObjectData(data: SuiObjectData): BitVector {
        if (data.bcs) {
            if (data.bcs.dataType !== "moveObject" || !isBitVector(data.bcs.type)) { throw new Error(`object at is not a BitVector object`); }

            return BitVector.fromBcs(fromB64(data.bcs.bcsBytes));
        } if (data.content) { return BitVector.fromSuiParsedData(data.content) } throw new Error("Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.");
    }

    static async fetch(client: SuiClient, id: string): Promise<BitVector> {
        const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching BitVector object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isBitVector(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a BitVector object`); }
