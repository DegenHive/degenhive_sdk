...
export class SuinsRegistration implements StructClass { __StructClass = true as const;
 ...
 static fromSuiObjectData( data: SuiObjectData ): SuinsRegistration { if (data.bcs) { if (data.bcs.dataType !== "moveObject" || !isSuinsRegistration(data.bcs.type)) { throw new Error(`object at is not a SuinsRegistration object`); }

 return SuinsRegistration.fromBcs( fromB64(data.bcs.bcsBytes) ); } if (data.content) { return SuinsRegistration.fromSuiParsedData( data.content ) } throw new Error( "Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request." ); }
