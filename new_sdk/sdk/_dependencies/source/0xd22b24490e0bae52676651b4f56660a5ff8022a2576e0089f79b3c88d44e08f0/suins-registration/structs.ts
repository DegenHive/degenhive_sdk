...
export class SuinsRegistration implements StructClass {
    ...
    private constructor(typeArgs: [], fields: SuinsRegistrationFields,) {
        this.$fullTypeName = composeSuiType(SuinsRegistration.$typeName, ...typeArgs) as `${typeof PKG_V1}::suins_registration::SuinsRegistration`; this.$typeArgs = typeArgs;
    ...
    static fromFieldsWithTypes(item: FieldsWithTypes): SuinsRegistration {
    ...
    static fromJSON(json: Record<string, any>): SuinsRegistration {
        if (json.$typeName !== SuinsRegistration.$typeName) { throw new Error("not a WithTwoGenerics json object") };
    ...
    static fromSuiObjectData(data: SuiObjectData): SuinsRegistration {
        if (data.bcs) {
            if (data.bcs.dataType !== "moveObject" || !isSuinsRegistration(data.bcs.type)) { throw new Error(`object at is not a SuinsRegistration object`); }

            return SuinsRegistration.fromBcs(fromB64(data.bcs.bcsBytes));
        } if (data.content) { return SuinsRegistration.fromSuiParsedData(data.content) } throw new Error("Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.");
    }

    static async fetch(client: SuiClient, id: string): Promise<SuinsRegistration> {
        const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching SuinsRegistration object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isSuinsRegistration(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a SuinsRegistration object`); }
