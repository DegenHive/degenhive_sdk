...
export class Domain implements StructClass {
    ...
    private constructor(typeArgs: [], fields: DomainFields,) {
        this.$fullTypeName = composeSuiType(Domain.$typeName, ...typeArgs) as `${typeof PKG_V1}::domain::Domain`; this.$typeArgs = typeArgs;
    ...
    static fromFieldsWithTypes(item: FieldsWithTypes): Domain {
    ...
    static fromJSON(json: Record<string, any>): Domain {
        if (json.$typeName !== Domain.$typeName) { throw new Error("not a WithTwoGenerics json object") };
    ...
    static fromSuiObjectData(data: SuiObjectData): Domain {
        if (data.bcs) {
            if (data.bcs.dataType !== "moveObject" || !isDomain(data.bcs.type)) { throw new Error(`object at is not a Domain object`); }

            return Domain.fromBcs(fromB64(data.bcs.bcsBytes));
        } if (data.content) { return Domain.fromSuiParsedData(data.content) } throw new Error("Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.");
    }

    static async fetch(client: SuiClient, id: string): Promise<Domain> {
        const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching Domain object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isDomain(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a Domain object`); }
