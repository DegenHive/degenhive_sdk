...
export function isTxContext(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V28}::tx_context::TxContext`; }
...
export class TxContext implements StructClass {
    ...
    static readonly $typeName = `${PKG_V28}::tx_context::TxContext`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

    readonly $typeName = TxContext.$typeName; readonly $fullTypeName: `${typeof PKG_V28}::tx_context::TxContext`; readonly $typeArgs: []; readonly $isPhantom = TxContext.$isPhantom;
    ...
    private constructor(typeArgs: [], fields: TxContextFields,) {
        this.$fullTypeName = composeSuiType(TxContext.$typeName, ...typeArgs) as `${typeof PKG_V28}::tx_context::TxContext`; this.$typeArgs = typeArgs;
    ...
    static fromJSON(json: Record<string, any>): TxContext {
        if (json.$typeName !== TxContext.$typeName) { throw new Error("not a WithTwoGenerics json object") };
    ...
    static fromSuiObjectData(data: SuiObjectData): TxContext {
        if (data.bcs) {
            if (data.bcs.dataType !== "moveObject" || !isTxContext(data.bcs.type)) { throw new Error(`object at is not a TxContext object`); }

            return TxContext.fromBcs(fromB64(data.bcs.bcsBytes));
        } if (data.content) { return TxContext.fromSuiParsedData(data.content) } throw new Error("Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.");
    }

    static async fetch(client: SuiClient, id: string): Promise<TxContext> {
        const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching TxContext object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isTxContext(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a TxContext object`); }
