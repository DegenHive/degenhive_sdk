...
export function isUrl(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V28}::url::Url`; }
...
export class Url implements StructClass {
    ...
    static readonly $typeName = `${PKG_V28}::url::Url`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

    readonly $typeName = Url.$typeName; readonly $fullTypeName: `${typeof PKG_V28}::url::Url`; readonly $typeArgs: []; readonly $isPhantom = Url.$isPhantom;

    readonly url: ToField<String>

    private constructor(typeArgs: [], fields: UrlFields,) {
        this.$fullTypeName = composeSuiType(Url.$typeName, ...typeArgs) as `${typeof PKG_V28}::url::Url`; this.$typeArgs = typeArgs;

        this.url = fields.url;
    }

    static reified(): UrlReified { return { typeName: Url.$typeName, fullTypeName: composeSuiType(Url.$typeName, ...[]) as `${typeof PKG_V28}::url::Url`, typeArgs: [] as [], isPhantom: Url.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Url.fromFields(fields,), fromFieldsWithTypes: (item: FieldsWithTypes) => Url.fromFieldsWithTypes(item,), fromBcs: (data: Uint8Array) => Url.fromBcs(data,), bcs: Url.bcs, fromJSONField: (field: any) => Url.fromJSONField(field,), fromJSON: (json: Record<string, any>) => Url.fromJSON(json,), fromSuiParsedData: (content: SuiParsedData) => Url.fromSuiParsedData(content,), fromSuiObjectData: (content: SuiObjectData) => Url.fromSuiObjectData(content,), fetch: async (client: SuiClient, id: string) => Url.fetch(client, id,), new: (fields: UrlFields,) => { return new Url([], fields) }, kind: "StructClassReified", } }
    ...
    static fromJSON(json: Record<string, any>): Url {
        if (json.$typeName !== Url.$typeName) { throw new Error("not a WithTwoGenerics json object") };
    ...
    static fromSuiObjectData(data: SuiObjectData): Url {
        if (data.bcs) {
            if (data.bcs.dataType !== "moveObject" || !isUrl(data.bcs.type)) { throw new Error(`object at is not a Url object`); }

            return Url.fromBcs(fromB64(data.bcs.bcsBytes));
        } if (data.content) { return Url.fromSuiParsedData(data.content) } throw new Error("Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.");
    }

    static async fetch(client: SuiClient, id: string): Promise<Url> {
        const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching Url object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isUrl(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a Url object`); }
