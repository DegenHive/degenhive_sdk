...
export class StakeSubsidy implements StructClass {
    ...
    private constructor(typeArgs: [], fields: StakeSubsidyFields,) {
        this.$fullTypeName = composeSuiType(StakeSubsidy.$typeName, ...typeArgs) as `${typeof PKG_V17}::stake_subsidy::StakeSubsidy`; this.$typeArgs = typeArgs;
    ...
    static fromJSON(json: Record<string, any>): StakeSubsidy {
        if (json.$typeName !== StakeSubsidy.$typeName) { throw new Error("not a WithTwoGenerics json object") };
    ...
    static fromSuiObjectData(data: SuiObjectData): StakeSubsidy {
        if (data.bcs) {
            if (data.bcs.dataType !== "moveObject" || !isStakeSubsidy(data.bcs.type)) { throw new Error(`object at is not a StakeSubsidy object`); }

            return StakeSubsidy.fromBcs(fromB64(data.bcs.bcsBytes));
        } if (data.content) { return StakeSubsidy.fromSuiParsedData(data.content) } throw new Error("Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.");
    }

    static async fetch(client: SuiClient, id: string): Promise<StakeSubsidy> {
        const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching StakeSubsidy object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isStakeSubsidy(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a StakeSubsidy object`); }
