...
export function isCurvedPoolConfig(type: string): boolean { type = compressSuiType(type); return type === "0x8dedf746c2648f520827d9a31604b86269afd0a4273b37b296742202ccfc306f::curved_math::CurvedPoolConfig"; }
...
export class CurvedPoolConfig implements StructClass { static readonly $typeName = "0x8dedf746c2648f520827d9a31604b86269afd0a4273b37b296742202ccfc306f::curved_math::CurvedPoolConfig"; static readonly $numTypeParams = 0;

 readonly $typeName = CurvedPoolConfig.$typeName;

 readonly $fullTypeName: "0x8dedf746c2648f520827d9a31604b86269afd0a4273b37b296742202ccfc306f::curved_math::CurvedPoolConfig";
 ...
 private constructor(typeArgs: [], fields: CurvedPoolConfigFields, ) { this.$fullTypeName = composeSuiType( CurvedPoolConfig.$typeName, ...typeArgs ) as "0x8dedf746c2648f520827d9a31604b86269afd0a4273b37b296742202ccfc306f::curved_math::CurvedPoolConfig"; this.$typeArgs = typeArgs;
 ...
 static reified( ): CurvedPoolConfigReified { return { typeName: CurvedPoolConfig.$typeName, fullTypeName: composeSuiType( CurvedPoolConfig.$typeName, ...[] ) as "0x8dedf746c2648f520827d9a31604b86269afd0a4273b37b296742202ccfc306f::curved_math::CurvedPoolConfig", typeArgs: [ ] as [], reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => CurvedPoolConfig.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => CurvedPoolConfig.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => CurvedPoolConfig.fromBcs( data, ), bcs: CurvedPoolConfig.bcs, fromJSONField: (field: any) => CurvedPoolConfig.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => CurvedPoolConfig.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => CurvedPoolConfig.fromSuiParsedData( content, ), fetch: async (client: SuiClient, id: string) => CurvedPoolConfig.fetch( client, id, ), new: ( fields: CurvedPoolConfigFields, ) => { return new CurvedPoolConfig( [], fields ) }, kind: "StructClassReified", } }
 ...
 static get bcs() { return bcs.struct("CurvedPoolConfig", {
 ...
 static fromFieldsWithTypes( item: FieldsWithTypes ): CurvedPoolConfig { if (!isCurvedPoolConfig(item.type)) { throw new Error("not a CurvedPoolConfig type");
 ...
 static fromJSON( json: Record<string, any> ): CurvedPoolConfig { if (json.$typeName !== CurvedPoolConfig.$typeName) { throw new Error("not a WithTwoGenerics json object") };
 ...
 static async fetch( client: SuiClient, id: string ): Promise<CurvedPoolConfig> { const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching CurvedPoolConfig object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isCurvedPoolConfig(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a CurvedPoolConfig object`); }
 return CurvedPoolConfig.fromBcs( fromB64(res.data.bcs.bcsBytes) ); }
