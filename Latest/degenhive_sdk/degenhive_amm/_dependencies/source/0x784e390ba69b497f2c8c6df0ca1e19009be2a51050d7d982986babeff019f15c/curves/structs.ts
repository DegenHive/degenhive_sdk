...
export class Stable implements StructClass { __StructClass = true as const;
 ...
 static reified( ): StableReified { return { typeName: Stable.$typeName, fullTypeName: composeSuiType( Stable.$typeName, ...[] ) as `${typeof PKG_V1}::curves::Stable`, typeArgs: [ ] as [], isPhantom: Stable.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Stable.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Stable.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => Stable.fromBcs( data, ), bcs: Stable.bcs, fromJSONField: (field: any) => Stable.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => Stable.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => Stable.fromSuiParsedData( content, ), fetch: async (client: SuiClient, id: string) => Stable.fetch( client, id, ), new: ( fields: StableFields, ) => { return new Stable( [], fields ) }, kind: "StructClassReified", } }
