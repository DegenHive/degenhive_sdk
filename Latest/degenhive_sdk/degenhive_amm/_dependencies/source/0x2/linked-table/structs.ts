...
export function isLinkedTable(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V21}::linked_table::LinkedTable` + '<'); }
...
export function isNode(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V21}::linked_table::Node` + '<'); }
...
export class Node<K extends TypeArgument, V extends TypeArgument> implements StructClass { __StructClass = true as const;
 ...
 static readonly $typeName = `${PKG_V21}::linked_table::Node`; static readonly $numTypeParams = 2; static readonly $isPhantom = [false,false,] as const;

 readonly $typeName = Node.$typeName; readonly $fullTypeName: `${typeof PKG_V21}::linked_table::Node<${ToTypeStr<K>}, ${ToTypeStr<V>}>`; readonly $typeArgs: [ToTypeStr<K>, ToTypeStr<V>]; readonly $isPhantom = Node.$isPhantom;

 readonly prev: ToField<Option<K>>; readonly next: ToField<Option<K>>; readonly value: ToField<V>

 private constructor(typeArgs: [ToTypeStr<K>, ToTypeStr<V>], fields: NodeFields<K, V>, ) { this.$fullTypeName = composeSuiType( Node.$typeName, ...typeArgs ) as `${typeof PKG_V21}::linked_table::Node<${ToTypeStr<K>}, ${ToTypeStr<V>}>`; this.$typeArgs = typeArgs;

 this.prev = fields.prev;; this.next = fields.next;; this.value = fields.value; }

 static reified<K extends Reified<TypeArgument, any>, V extends Reified<TypeArgument, any>>( K: K, V: V ): NodeReified<ToTypeArgument<K>, ToTypeArgument<V>> { return { typeName: Node.$typeName, fullTypeName: composeSuiType( Node.$typeName, ...[extractType(K), extractType(V)] ) as `${typeof PKG_V21}::linked_table::Node<${ToTypeStr<ToTypeArgument<K>>}, ${ToTypeStr<ToTypeArgument<V>>}>`, typeArgs: [ extractType(K), extractType(V) ] as [ToTypeStr<ToTypeArgument<K>>, ToTypeStr<ToTypeArgument<V>>], isPhantom: Node.$isPhantom, reifiedTypeArgs: [K, V], fromFields: (fields: Record<string, any>) => Node.fromFields( [K, V], fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Node.fromFieldsWithTypes( [K, V], item, ), fromBcs: (data: Uint8Array) => Node.fromBcs( [K, V], data, ), bcs: Node.bcs(toBcs(K), toBcs(V)), fromJSONField: (field: any) => Node.fromJSONField( [K, V], field, ), fromJSON: (json: Record<string, any>) => Node.fromJSON( [K, V], json, ), fromSuiParsedData: (content: SuiParsedData) => Node.fromSuiParsedData( [K, V], content, ), fetch: async (client: SuiClient, id: string) => Node.fetch( client, [K, V], id, ), new: ( fields: NodeFields<ToTypeArgument<K>, ToTypeArgument<V>>, ) => { return new Node( [extractType(K), extractType(V)], fields ) }, kind: "StructClassReified", } }
