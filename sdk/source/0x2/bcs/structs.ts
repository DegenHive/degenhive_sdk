...
export function isBCS(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V28}::bcs::BCS`; }
...
export class BCS implements StructClass {
    ...
    static readonly $typeName = `${PKG_V28}::bcs::BCS`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

    readonly $typeName = BCS.$typeName; readonly $fullTypeName: `${typeof PKG_V28}::bcs::BCS`; readonly $typeArgs: []; readonly $isPhantom = BCS.$isPhantom;

    readonly bytes: ToField<Vector<"u8">>

    private constructor(typeArgs: [], fields: BCSFields,) {
        this.$fullTypeName = composeSuiType(BCS.$typeName, ...typeArgs) as `${typeof PKG_V28}::bcs::BCS`; this.$typeArgs = typeArgs;

        this.bytes = fields.bytes;
    }

    static reified(): BCSReified { return { typeName: BCS.$typeName, fullTypeName: composeSuiType(BCS.$typeName, ...[]) as `${typeof PKG_V28}::bcs::BCS`, typeArgs: [] as [], isPhantom: BCS.$isPhantom, reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => BCS.fromFields(fields,), fromFieldsWithTypes: (item: FieldsWithTypes) => BCS.fromFieldsWithTypes(item,), fromBcs: (data: Uint8Array) => BCS.fromBcs(data,), bcs: BCS.bcs, fromJSONField: (field: any) => BCS.fromJSONField(field,), fromJSON: (json: Record<string, any>) => BCS.fromJSON(json,), fromSuiParsedData: (content: SuiParsedData) => BCS.fromSuiParsedData(content,), fromSuiObjectData: (content: SuiObjectData) => BCS.fromSuiObjectData(content,), fetch: async (client: SuiClient, id: string) => BCS.fetch(client, id,), new: (fields: BCSFields,) => { return new BCS([], fields) }, kind: "StructClassReified", } }
    ...
    static fromJSON(json: Record<string, any>): BCS {
        if (json.$typeName !== BCS.$typeName) { throw new Error("not a WithTwoGenerics json object") };
    ...
    static fromSuiObjectData(data: SuiObjectData): BCS {
        if (data.bcs) {
            if (data.bcs.dataType !== "moveObject" || !isBCS(data.bcs.type)) { throw new Error(`object at is not a BCS object`); }

            return BCS.fromBcs(fromB64(data.bcs.bcsBytes));
        } if (data.content) { return BCS.fromSuiParsedData(data.content) } throw new Error("Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.");
    }

    static async fetch(client: SuiClient, id: string): Promise<BCS> {
        const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching BCS object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isBCS(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a BCS object`); }
