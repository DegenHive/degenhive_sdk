...
export function isExtension(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V28}::kiosk_extension::Extension`; }
...
export class Extension implements StructClass {
    ...
    static readonly $typeName = `${PKG_V28}::kiosk_extension::Extension`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

    readonly $typeName = Extension.$typeName; readonly $fullTypeName: `${typeof PKG_V28}::kiosk_extension::Extension`; readonly $typeArgs: []; readonly $isPhantom = Extension.$isPhantom;
    ...
    static fromJSON(json: Record<string, any>): Extension {
        if (json.$typeName !== Extension.$typeName) { throw new Error("not a WithTwoGenerics json object") };
    ...
    static fromSuiObjectData(data: SuiObjectData): Extension {
        if (data.bcs) {
            if (data.bcs.dataType !== "moveObject" || !isExtension(data.bcs.type)) { throw new Error(`object at is not a Extension object`); }

            return Extension.fromBcs(fromB64(data.bcs.bcsBytes));
        } if (data.content) { return Extension.fromSuiParsedData(data.content) } throw new Error("Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.");
    }

    static async fetch(client: SuiClient, id: string): Promise<Extension> {
        const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching Extension object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isExtension(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a Extension object`); }
...
export function isExtensionKey(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V28}::kiosk_extension::ExtensionKey` + '<'); }
...
export class ExtensionKey<Ext extends PhantomTypeArgument> implements StructClass {
    ...
    static fromJSON<Ext extends PhantomReified<PhantomTypeArgument>>(typeArg: Ext, json: Record<string, any>): ExtensionKey<ToPhantomTypeArgument<Ext>> {
        if (json.$typeName !== ExtensionKey.$typeName) { throw new Error("not a WithTwoGenerics json object") }; assertReifiedTypeArgsMatch(composeSuiType(ExtensionKey.$typeName, extractType(typeArg)), json.$typeArgs, [typeArg],)
    ...
    static fromSuiObjectData<Ext extends PhantomReified<PhantomTypeArgument>>(typeArg: Ext, data: SuiObjectData): ExtensionKey<ToPhantomTypeArgument<Ext>> {
        if (data.bcs) {
            if (data.bcs.dataType !== "moveObject" || !isExtensionKey(data.bcs.type)) { throw new Error(`object at is not a ExtensionKey object`); }
        ...
        } if (data.content) { return ExtensionKey.fromSuiParsedData(typeArg, data.content) } throw new Error("Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.");
    }

    static async fetch<Ext extends PhantomReified<PhantomTypeArgument>>(client: SuiClient, typeArg: Ext, id: string): Promise<ExtensionKey<ToPhantomTypeArgument<Ext>>> {
        const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching ExtensionKey object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isExtensionKey(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a ExtensionKey object`); }
