...
export function isWrapper(type: string): boolean { type = compressSuiType(type); return type.startsWith(`${PKG_V28}::dynamic_object_field::Wrapper` + '<'); }
...
export class Wrapper<Name extends TypeArgument> implements StructClass {
    ...
    static fromJSON<Name extends Reified<TypeArgument, any>>(typeArg: Name, json: Record<string, any>): Wrapper<ToTypeArgument<Name>> {
        if (json.$typeName !== Wrapper.$typeName) { throw new Error("not a WithTwoGenerics json object") }; assertReifiedTypeArgsMatch(composeSuiType(Wrapper.$typeName, extractType(typeArg)), json.$typeArgs, [typeArg],)
    ...
    static fromSuiObjectData<Name extends Reified<TypeArgument, any>>(typeArg: Name, data: SuiObjectData): Wrapper<ToTypeArgument<Name>> {
        if (data.bcs) {
            if (data.bcs.dataType !== "moveObject" || !isWrapper(data.bcs.type)) { throw new Error(`object at is not a Wrapper object`); }
        ...
        } if (data.content) { return Wrapper.fromSuiParsedData(typeArg, data.content) } throw new Error("Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.");
    }

    static async fetch<Name extends Reified<TypeArgument, any>>(client: SuiClient, typeArg: Name, id: string): Promise<Wrapper<ToTypeArgument<Name>>> {
        const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching Wrapper object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isWrapper(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a Wrapper object`); }
