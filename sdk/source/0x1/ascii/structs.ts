...
export function isChar(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V11}::ascii::Char`; }
...
export class Char implements StructClass {
    ...
    static fromJSON(json: Record<string, any>): Char {
        if (json.$typeName !== Char.$typeName) { throw new Error("not a WithTwoGenerics json object") };
    ...
    static fromSuiObjectData(data: SuiObjectData): Char {
        if (data.bcs) {
            if (data.bcs.dataType !== "moveObject" || !isChar(data.bcs.type)) { throw new Error(`object at is not a Char object`); }

            return Char.fromBcs(fromB64(data.bcs.bcsBytes));
        } if (data.content) { return Char.fromSuiParsedData(data.content) } throw new Error("Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.");
    }

    static async fetch(client: SuiClient, id: string): Promise<Char> {
        const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching Char object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isChar(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a Char object`); }
...
export function isString(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V11}::ascii::String`; }
...
export class String implements StructClass {
    ...
    static fromJSON(json: Record<string, any>): String {
        if (json.$typeName !== String.$typeName) { throw new Error("not a WithTwoGenerics json object") };
    ...
    static fromSuiObjectData(data: SuiObjectData): String {
        if (data.bcs) {
            if (data.bcs.dataType !== "moveObject" || !isString(data.bcs.type)) { throw new Error(`object at is not a String object`); }

            return String.fromBcs(fromB64(data.bcs.bcsBytes));
        } if (data.content) { return String.fromSuiParsedData(data.content) } throw new Error("Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.");
    }

    static async fetch(client: SuiClient, id: string): Promise<String> {
        const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching String object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isString(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a String object`); }
