...
export function isUnverifiedValidatorOperationCap(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V17}::validator_cap::UnverifiedValidatorOperationCap`; }
...
export class UnverifiedValidatorOperationCap implements StructClass {
    ...
    static readonly $typeName = `${PKG_V17}::validator_cap::UnverifiedValidatorOperationCap`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

    readonly $typeName = UnverifiedValidatorOperationCap.$typeName; readonly $fullTypeName: `${typeof PKG_V17}::validator_cap::UnverifiedValidatorOperationCap`; readonly $typeArgs: []; readonly $isPhantom = UnverifiedValidatorOperationCap.$isPhantom;
    ...
    static fromJSON(json: Record<string, any>): UnverifiedValidatorOperationCap {
        if (json.$typeName !== UnverifiedValidatorOperationCap.$typeName) { throw new Error("not a WithTwoGenerics json object") };
    ...
    static fromSuiObjectData(data: SuiObjectData): UnverifiedValidatorOperationCap {
        if (data.bcs) {
            if (data.bcs.dataType !== "moveObject" || !isUnverifiedValidatorOperationCap(data.bcs.type)) { throw new Error(`object at is not a UnverifiedValidatorOperationCap object`); }

            return UnverifiedValidatorOperationCap.fromBcs(fromB64(data.bcs.bcsBytes));
        } if (data.content) { return UnverifiedValidatorOperationCap.fromSuiParsedData(data.content) } throw new Error("Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.");
    }
...
export function isValidatorOperationCap(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V17}::validator_cap::ValidatorOperationCap`; }
...
export class ValidatorOperationCap implements StructClass {
    ...
    static fromJSON(json: Record<string, any>): ValidatorOperationCap {
        if (json.$typeName !== ValidatorOperationCap.$typeName) { throw new Error("not a WithTwoGenerics json object") };
    ...
    static fromSuiObjectData(data: SuiObjectData): ValidatorOperationCap {
        if (data.bcs) {
            if (data.bcs.dataType !== "moveObject" || !isValidatorOperationCap(data.bcs.type)) { throw new Error(`object at is not a ValidatorOperationCap object`); }

            return ValidatorOperationCap.fromBcs(fromB64(data.bcs.bcsBytes));
        } if (data.content) { return ValidatorOperationCap.fromSuiParsedData(data.content) } throw new Error("Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.");
    }
