...
export function isStorageFund(type: string): boolean { type = compressSuiType(type); return type === `${PKG_V17}::storage_fund::StorageFund`; }
...
export class StorageFund implements StructClass {
    ...
    static readonly $typeName = `${PKG_V17}::storage_fund::StorageFund`; static readonly $numTypeParams = 0; static readonly $isPhantom = [] as const;

    readonly $typeName = StorageFund.$typeName; readonly $fullTypeName: `${typeof PKG_V17}::storage_fund::StorageFund`; readonly $typeArgs: []; readonly $isPhantom = StorageFund.$isPhantom;
    ...
    static fromJSON(json: Record<string, any>): StorageFund {
        if (json.$typeName !== StorageFund.$typeName) { throw new Error("not a WithTwoGenerics json object") };
    ...
    static fromSuiObjectData(data: SuiObjectData): StorageFund {
        if (data.bcs) {
            if (data.bcs.dataType !== "moveObject" || !isStorageFund(data.bcs.type)) { throw new Error(`object at is not a StorageFund object`); }

            return StorageFund.fromBcs(fromB64(data.bcs.bcsBytes));
        } if (data.content) { return StorageFund.fromSuiParsedData(data.content) } throw new Error("Both `bcs` and `content` fields are missing from the data. Include `showBcs` or `showContent` in the request.");
    }

    static async fetch(client: SuiClient, id: string): Promise<StorageFund> {
        const res = await client.getObject({ id, options: { showBcs: true, }, }); if (res.error) { throw new Error(`error fetching StorageFund object at id ${id}: ${res.error.code}`); } if (res.data?.bcs?.dataType !== "moveObject" || !isStorageFund(res.data.bcs.type)) { throw new Error(`object at id ${id} is not a StorageFund object`); }
