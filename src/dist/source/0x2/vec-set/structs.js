...
/* ============================== VecSet =============================== */
function isVecSet(type) { type = (0, util_1.compressSuiType)(type); return type.startsWith(`${index_1.PKG_V28}::vec_set::VecSet` + '<'); }
class VecSet {
    ...
    static reified(K) { return { typeName: VecSet.$typeName, fullTypeName: (0, util_1.composeSuiType)(VecSet.$typeName, ...[(0, reified_1.extractType)(K)]), typeArgs: [(0, reified_1.extractType)(K)], isPhantom: VecSet.$isPhantom, reifiedTypeArgs: [K], fromFields: (fields) => VecSet.fromFields(K, fields), fromFieldsWithTypes: (item) => VecSet.fromFieldsWithTypes(K, item), fromBcs: (data) => VecSet.fromBcs(K, data), bcs: VecSet.bcs((0, reified_1.toBcs)(K)), fromJSONField: (field) => VecSet.fromJSONField(K, field), fromJSON: (json) => VecSet.fromJSON(K, json), fromSuiParsedData: (content) => VecSet.fromSuiParsedData(K, content), fromSuiObjectData: (content) => VecSet.fromSuiObjectData(K, content), fetch: async (client, id) => VecSet.fetch(client, K, id), new: (fields) => { return new VecSet([(0, reified_1.extractType)(K)], fields); }, kind: "StructClassReified", }; }
    ...
    static fromSuiObjectData(typeArg, data) {
        if (data.bcs) {
            ...
            ;
            return VecSet.fromBcs(typeArg, (0, bcs_1.fromB64)(data.bcs.bcsBytes));
        }
        if (data.content) {
...
VecSet.$typeName = `${index_1.PKG_V28}::vec_set::VecSet`;
